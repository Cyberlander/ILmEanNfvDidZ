...
def get_data_for_nn_meta_function( datapath, sequence_len, input_vector_dim ):
    ...
    if y_label.shape[1]< 3:
        last_column = np.zeros((y_label.shape[0],1))
        y_label = np.concatenate( ( y_label, last_column  ), axis=1 )
...
if __name__ == "__main__":
    ...
    CHUNK_DATABASE_PATH = "../../datasets_for_experiments/facebook_chunks/fb_chunk_{}.csv"
    ...
    for i in range(1,51):
        ...
        HOLDOUT_PERCENT = 0.2
        _, X_holdout, _, y_holdout = train_test_split( X_sources_sentences_1, y_label_1, test_size=HOLDOUT_PERCENT, random_state=42)
