...
if __name__ == "__main__":
    ...
    with open( "no_datapool_verlauf_2_nets_1_testset.txt", "w" ) as f:
        f.write(str(ACCURACIES))
        f.write(str(PRECISIONS))
        f.write(str(RECALLS))
        f.write(str(F1S))    
        #nn_model.save_weights( "weights_basemodel/model.h5" )

        #X_train_meta_1 = nn_model_1.predict( X_holdout_1 )
        #y_train_meta_1 = categorical_to_numeric( y_holdout_1 )
        #X_train_meta_2 = nn_model_2.predict( X_holdout_2 )
        #y_train_meta_2 = categorical_to_numeric( y_holdout_2 )

    #    if i == 0:
        #    X_train_meta = np.concatenate( ( X_train_meta_1, X_train_meta_2 ), axis=0 )
        #    y_train_meta = np.concatenate( ( y_train_meta_1, y_train_meta_2 ), axis=0 )
        #else:
            #X_train_meta = np.concatenate( ( X_train_meta, X_train_meta_1, X_train_meta_2 ), axis=0 )
        #    y_train_meta = np.concatenate( ( y_train_meta, y_train_meta_1, y_train_meta_2 ), axis=0 )
            #print("SHAPE"*10)
