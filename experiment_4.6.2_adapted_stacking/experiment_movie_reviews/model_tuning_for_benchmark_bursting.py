...
def get_cnn_model_to_burst_benchmark( sequence_len, vector_dim, num_classes ):
    model = Sequential()
    model.add( Conv1D( filters=1000,input_shape=(sequence_len, vector_dim), kernel_size=3, padding='same', activation='relu', name='conv_layer_1' ))
    model.add( GlobalAveragePooling1D(data_format='channels_last') )
    model.add( Dense(128, activation='relu') )
    model.add( layers.Dropout(0.5))
    model.add( Dense( num_classes, activation='sigmoid'))
...
if __name__ == "__main__":
    ...
    X_train, y_train = get_data_for_nn_meta_function( "../../datasets_for_experiments/movie_reviews_10_to_burst_benchmark/movie_reviews_train.csv", SEQUENCE_LEN, INPUT_VECTOR_DIM )
