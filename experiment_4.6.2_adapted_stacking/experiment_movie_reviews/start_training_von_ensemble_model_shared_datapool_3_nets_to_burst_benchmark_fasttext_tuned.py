...
def categorical_to_numeric( y_data ):
    ...
    for element in y_data:
        #print("Element:", element, "Argmax:", np.argmax( element))
...
if __name__ == "__main__":
    ...
    RECALLS = []
    F1S = []
    X_sources_sentences_complete, y_label_complete = get_data_for_nn_meta_function(  "../../datasets_for_experiments/movie_reviews_10_to_burst_benchmark/movie_reviews_train_shuffled.csv", SEQUENCE_LEN, INPUT_VECTOR_DIM )
    ...
    for i in range(10):
        ...
        y_to_be_predicted = list(y_holdout )
        ...
        X_train_meta = np.zeros((X_train_meta_1.shape[0],3))
        X_train_meta[:,0] = X_train_meta_1.reshape((-1,))
        X_train_meta[:,1] = X_train_meta_2.reshape((-1,))
        X_train_meta[:,2] = X_train_meta_3.reshape((-1,))
        ...
        X_test_meta = np.zeros((X_test_meta_1.shape[0],3))
        X_test_meta[:,0] = X_test_meta_1.reshape((-1,))
        X_test_meta[:,1] = X_test_meta_2.reshape((-1,))
        X_test_meta[:,2] = X_test_meta_3.reshape((-1,))
    ...
    with open( "verlauf_3_nets_testset_1_to_burst_benchmark.txt", "w" ) as f:
